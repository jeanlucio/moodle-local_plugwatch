...
/**
 * Tests for the local_plugwatch pre-uninstallation hook.
 *
...
/**
 * Tests for xmldb_local_plugwatch_uninstall(). Every table in db/install.xml is
 * dropped automatically by core, so the only thing worth exercising here is the one
 * piece of cleanup core does not do for us: user_preferences rows.
 *
 ...
 * this test guards against a future accidental rename dropping the "local_" prefix.
 *
 * @covers ::xmldb_local_plugwatch_uninstall
 */
final class uninstall_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        parent::setUp();
        $this->resetAfterTest();
        require_once($CFG->dirroot . '/local/plugwatch/db/uninstall.php');
    }

    /**
     * Tests that the uninstall hook deletes only local_plugwatch-prefixed
    ...
    public function test_uninstall_with_no_matching_preferences_is_a_noop(): void {
        $this->assertTrue(xmldb_local_plugwatch_uninstall());
    }
}
